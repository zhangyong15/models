...
def BidirectionEncoderLstmmemory(name, input, size, depth, with_attention):
    ...
    if not with_attention:
        Layer(
            ...
            bias=False,
            inputs=[
                Input("{n}_forward_lstm{idx:02d}".format(
                    n=name, idx=depth))
            ])
        Layer(
            ...
            bias=False,
            inputs=[
                Input("{n}_backward_lstm{idx:02d}".format(
                    n=name, idx=depth))
            ])
...
if generating:
    ...
    Inputs("src_word", "trg_word", "trg_next_word")
    Outputs("cost")
...
if generating:
    ...
    MixedLayer(
        ...
        inputs=TableProjection(
            ...
            parameter_name="_emb"), )
decoder_input = "predicted_emb" if generating else "trg_emb"
...
decoder_state_memory = Memory(
    name="decoder_unit01", size=latent_chain_dim, is_sequence=False)
...
for i in range(1, decoder_depth + 1):
    ...
    recurrent.LstmRecurrentUnit(
        ...
        inputs=[
            IdentityProjection("decoder_input_recurrent{idx:02d}".format(idx=i))
        ],
        para_prefix="decoder_unit{idx:02d}".format(idx=i),
        error_clipping_threshold=clipping_thd,
        out_memory=decoder_state_memory if i == 1 else None)
...
Layer(
    ...
    active_type="softmax",
    bias=Bias(
        parameter_name="_output.wbias", initial_std=0),
    inputs=FullMatrixProjection("decoder_unit{idx:02d}".format(
        idx=decoder_depth)))
